...
3. **Unsubscribe protection.** Reply-based opt-outs remain enabled. A visible unsubscribe link must be present in promotional outreach. Mailshake's visible-link One-click/Two-click setting is never treated as proof of Gmail/Yahoo RFC 8058 compliance. Before any Franklin Navigator promotional outreach campaign is released from its compliance hold, send a controlled real message to an owner-controlled mailbox and inspect the received raw headers. The message must contain a valid `List-Unsubscribe` HTTPS URL header and `List-Unsubscribe-Post: List-Unsubscribe=One-Click`. If either is missing or malformed, the campaign remains paused until the sending method is corrected or replaced. Opt-outs are durable suppressions across remaps, imports, campaigns, and provider migrations.
...
21. **RFC 8058 proof gate.** Do not infer compliance from provider marketing, UI labels, or a body unsubscribe link. Verify an actual received message's raw headers before releasing promotional outreach. Required proof: `List-Unsubscribe` contains an HTTPS unsubscribe URL and `List-Unsubscribe-Post` equals `List-Unsubscribe=One-Click`. Re-verify after any material provider, mailbox, SMTP, domain, or campaign-delivery architecture change.
